...
async def view_profiles(message: Message, state: FSMContext):
    ...
    print(profile[0])
    await message.answer_photo(profile[6], caption=response, reply_markup=inline.get_profile_inline_kb(profile[0]))
...
async def handle_vote(callback: CallbackQuery):
    ...
    with sqlite3.connect("database.db") as db:
        ...
        if action == "like":
            cursor.execute("INSERT OR IGNORE INTO likes(from_user, to_user) VALUES (?, ?)",
                   (from_user_id, to_user_id))
            cursor.execute("SELECT * FROM likes WHERE from_user = ? AND to_user = ?",
            ...
            if cursor.fetchone():
                ...
                cursor.execute("SELECT name, age, bio, photo_file_id FROM profiles WHERE user_id = ?", (from_user_id,))
                ...
                if sender_profile:
                    ...
                    await callback.bot.send_message(
                        chat_id=to_user_id,
                        text=f"❤️ Кто-то хочет с вами познакомиться!"
                    )
            ...
            cursor.execute("INSERT OR REPLACE INTO timeouts(user_id, timeout_until) VALUES (?, ?)",
                           (to_user_id, timeout_until))
            cursor.execute("INSERT INTO dislikes(from_user, to_user, timestamp) VALUES (?, ?, datetime('now'))",
                           (from_user_id, to_user_id))
